...
void startCameraServer()
{
    ...
    config.ctrl_port = 85; // Keep control far away from audio ports
    config.stack_size = 8192;       // More memory for the web server
    config.max_open_sockets = 7;
    config.lru_purge_enable = true; 

    // Define all the URIs first
    ...
    Serial.println("Starting Camera Server on port 81...");
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        Serial.println("CAMERA SERVER ONLINE AT PORT 81");
    }
    else
    {
        Serial.println("FAILED TO START SERVER ON 81");
    }

    // Initialize the filter used for the stream
